function opm_gamma_avg_TFS_AAL(sub,ses,project_dir)
%5 if running
% sub = '016'
% ses = '001'
...
%% Get TFS from peak voxel locations AAL
VE_AAL = [];
for voxind = 1:size(Lead_fields,3)
    this_L = Lead_fields(:,:,voxind);
    ...
    VE_AAL(:,voxind) = VE_unchopped;
    ind = (1:3*fs:length(trial_info.start)*3*fs)-1;
end
cd(path_TFS)
save(files_VE_AAL,'VE_AAL','ind','trial_time','fs');
