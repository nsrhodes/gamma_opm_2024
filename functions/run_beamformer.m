...
path.meshes = [project_dir,'derivatives',filesep,'sourcespace',filesep,sub,...
    filesep,sub,'_meshes.mat'];
...
path.segmentedmri = [project_dir,'derivatives',filesep,'sourcespace',filesep,sub,...
...
elseif strcmpi(fwd_method,'local')
    ...
    [path.skanect, file_name.skanect] = fileparts(S.skanect_file);
    path.skanect = [path.skanect filesep];
    
    % Create spheres
    if exist([path.skanect,'local_spheres_info_',file_name.skanect(end-5:end),'.mat'])
        load([path.skanect,'local_spheres_info_',file_name.skanect(end-5:end),'.mat']);
    else
        ...
        save([path.skanect,'local_spheres_info_',file_name.skanect(end-5:end),'.mat'],...
...
elseif strcmpi(fwd_method,'shell')
    ...
    [LF_single_shell,L_reshaped] = Forward_OPM_shell_FT([path.mri,filesep,file_name.mri,'.nii'],sensor_info,sourcepos);
    LF1 = L_reshaped.*1e-9;
