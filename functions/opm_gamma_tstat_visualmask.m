function opm_gamma_tstat_visualmask(sub,ses,project_dir,power_line,age)
%5 if running
clear all
close all
sub = '016'
...
VOI_mat_file = [path_meshes,files_VOI];
    ...
    VOI = (AAL_regions.anatomy(:,:,:,22) > 0) | (AAL_regions.anatomy(:,:,:,23)>0) | ...
        (AAL_regions.anatomy(:,:,:,24) > 0) | (AAL_regions.anatomy(:,:,:,25)>0) | ...
        (AAL_regions.anatomy(:,:,:,26) > 0) | (AAL_regions.anatomy(:,:,:,61)>0) | ...
        (AAL_regions.anatomy(:,:,:,62) > 0) | (AAL_regions.anatomy(:,:,:,63)>0) | ...
        (AAL_regions.anatomy(:,:,:,64) > 0) | (AAL_regions.anatomy(:,:,:,65) > 0);
    % dilate and crop using union of AAL regions
    SE = strel('sphere',5)
...
if ~cleaning_only
    ...
    active_window = round([0.1 0.9].*fs-trig_offset*fs);active_inds = active_window(1):active_window(2);
    control_window = round([-0.9 -0.1].*fs-trig_offset*fs);control_inds = control_window(1):control_window(2);
